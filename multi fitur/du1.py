...
commands = {  # command description used in the "help" command
    ...
    'media'      : 'mengirim anda musik,foto,video'
}

imageSelect = types.ReplyKeyboardMarkup(one_time_keyboard=True)  # create the image selection keyboard
imageSelect.add('ayam', 'video', 'musik','kadal')
...
def msg_image_select(m):
    ...
    if text == 'ayam':  # send the appropriate image based on the reply to the "/getImage" command
        ...
        bot.send_video(cid, open('ojol.mp4', 'rb'), reply_markup=hideBoard)
        # userStep[cid] = 0
    elif text == 'musik':
        bot.send_audio(cid, open('let.mp3', 'rb'), reply_markup=hideBoard)
        # userStep[cid] = 0
    elif text == 'kadal':
